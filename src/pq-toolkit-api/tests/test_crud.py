...
def test_add_experiment(session, create_experiment):
    ...
    retrieved_experiment = session.exec(
...
def test_upload_experiment_config(
    session, create_experiment, upload_config, experiment_data
):
    experiment_name = "Test Experiment"
    create_experiment(experiment_name)
...
def test_remove_experiment_by_name(
    session, create_experiment, upload_config, experiment_data
):
    experiment_name = "Test Experiment"
    create_experiment(experiment_name)
...
@pytest.mark.parametrize(
    "config_data, test_result, expected_result_type",
    [
        (
            {
...
def test_update_experiment_config(
    session, create_experiment, upload_config, experiment_data, updated_experiment_data
):
    experiment_name = "Test Experiment"
    create_experiment(experiment_name)
    upload_config(experiment_name, experiment_data)
    ...
    assert (
        updated_experiment.tests[1].test_number
        == updated_experiment_data["tests"][1]["test_number"]
    )


@pytest.mark.parametrize(
    "config_data, test_result, expected_error",
    [
        (
...
def test_add_experiment_results_invalid(
    session, create_experiment, upload_config, config_data, test_result, expected_error
):
    experiment_name = "Test Experiment"
    create_experiment(experiment_name)
    upload_config(experiment_name, config_data)

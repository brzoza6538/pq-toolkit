...
const MultiPlayer = ({
  ...
  const length = Math.round(getPlayerLength(playersRef.current[selectedPlayer]))
    ...
    >
      ...
      >
        <TableBody>
		  ...
		  <TableRow  className="mt-sm gap-sm w-full">
			<TableCell className="h-1" rowSpan={footerLevels}>{scale}</TableCell>
			{Array.from(assets.keys()).map((name) => {
			const footer: JSX.Element | undefined = assets
			  .get(name)
			  ?.footers?.at(0)
			return (
			  <TableCell key={name} className="h-2">
			  {footer}
			  </TableCell>
			)
		  })}
		  </TableRow>
		  {Array.from(Array(footerLevels - 1).keys()).map((idx) => (
		    <TableRow key={idx + 1} className="mt-sm gap-sm w-full">
		    {Array.from(assets.keys()).map((name) => {
			  const footer: JSX.Element | undefined = assets
			  .get(name)
			  ?.footers?.at(idx + 1)

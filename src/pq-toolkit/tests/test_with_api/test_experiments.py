...
from pqtoolkit.dataclasses import (
    PqExperiment,
    PqTestAB,
    PqTestMUSHRA,
    PqSample,
...
from pqtoolkit.exceptions import PqExperimentSetupException
...
class TestExperimentsWithAPI(unittest.TestCase):
    ...
    def test_setup_experiment(self):
        ...
        self.client.setup_experiment(
            experiment_name=experiment_name, experiment_setup=experiment_setup
        )

        experiment = self.client.get_experiment(experiment_name=experiment_name)
        assert experiment.model_dump(exclude={"uid"}) == experiment_setup.model_dump(
            exclude={"uid"}
        )
